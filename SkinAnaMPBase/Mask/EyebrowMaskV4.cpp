                       ...
                       PolarContour& evenPolarSeq)
{
    ...
    PtInPolarSeq ccSeq = MakePolarSeqInCCWise(posiThetaPtSeq);
    ...
    PtInPolarSeq seqStartZeroTheta;
    while(curID <= ccSeq.size() - 1)
    {
        seqStartZeroTheta.push_back(ccSeq[curID]);
        curID++;
    }
                        ...
                        POLYGON& eyePg)
{
    ...
    BuildEvenPolarSeq(rawPolarSeq,
                      72, // how many intervals from 0 to 2*Pi
                      evenPolarSeq);
}
