...
#pragma omp section
        {
            gaborBank[0]->conv_img(normImg, temp1, 1);  // lRegion is source, temp1 is destination
        }
#pragma omp section
        {
            gaborBank[1]->conv_img(normImg, temp2, 1);
        }
#pragma omp section
        {
            gaborBank[2]->conv_img(normImg, temp3, 1);
        }
#pragma omp section
        {
            gaborBank[3]->conv_img(normImg, temp4, 1);
        }
#pragma omp section
        {
            gaborBank[4]->conv_img(normImg, temp5, 1);
        }
    }
    
    //函数原型： void cv::min(InputArray  src1, InputArray  src2, OutputArray  dst)
    //功   能： 单像素操作（与领域无关）；将src1和src2同位置的像素值取小者，存贮到dst中
    cv::Mat regResp(detRegImg.size(), CV_8S, cv::Scalar(127)); // 8S: 8位有符号数
...
// 返回左面颊的Gabor滤波响应值
Mat CalcGaborRespInOneCheek(vector<CvGabor*> gaborBank,
                  const Mat& grSrcImg,
                  Rect& cheekRect)
{
                   ...
                   Mat& gaborRespMap)
{
    ...
    Mat lCheekResp = CalcGaborRespInOneCheek(lGaborBank, grSrcImg, wrkRegGroup.lCheekReg);

    // 计算右面颊的Gabor滤波响应值
    vector<CvGabor*> rGaborBank;
    InitRCheekGaborBank(rGaborBank);
    Mat rCheekResp = CalcGaborRespInOneCheek(rGaborBank, grSrcImg, wrkRegGroup.rCheekReg);
